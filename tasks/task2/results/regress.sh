...
timeout 2 bash -c "</dev/tcp/${DB_HOST}/5434" \
  || { echo "❌ Не удалось подключиться к ${DB_HOST}:5434"; exit 1; }

# Загрузка фикстур
echo "🧪 Загрузка фикстур..."
PGPASSWORD="${DB_PASSWORD}" psql -h "${DB_HOST}" -p "${DB_PORT}" -U "${DB_USER}" "${DB_NAME}" < init-fixtures.sql
echo "🧪 Загрузка фикстур (booking)..."
PGPASSWORD="booking" psql -h "${DB_HOST}" -p "5434" -U "booking" "booking" < init-fixtures-booking.sql
...
echo "SKIPPED Нерабочий тест по логике p-o-y-1.0.0.jar"
...
# Проверим появление исторических записей
PGPASSWORD="booking_history" psql -h "${DB_HOST}" -p "5435" -U "booking_history" \
-d "booking_history" -c "select * from booking_hist_rec" | grep -q test-hotel-1 \
&& pass "Исторические записи есть" || fail "Исторических записей нет"
